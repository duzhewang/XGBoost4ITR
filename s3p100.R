...
# Scenario 3: polynomial decision rule 

rm(list=ls())
set.seed(2019)
funcdir="/Users/duzhewang/Desktop/itrsimu/func/"
savedir<-"/Users/duzhewang/Desktop/itrsimu/s3p100/" 
...
datagen<-function(n, p){
  ...
  y<- 1+2*X[,1]+X[,2]+0.5*X[,3]+(0.2+X[,1]*X[,1]+X[,2]*X[,2]-X[,3]*X[,3]-X[,4]*X[,4])*A+rnorm(n)
...
trueoptITR<-function(X){
  trueoptITR<-(0.2+X[,1]*X[,1]+X[,2]*X[,2]-X[,3]*X[,3]-X[,4]*X[,4]>0)*2-1   
  return(trueoptITR)
}
