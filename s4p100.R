...
# Scenario 4: circle decision rule   

rm(list=ls())
set.seed(2019)
funcdir="/Users/duzhewang/Desktop/itrsimu/func/"
savedir<-"/Users/duzhewang/Desktop/itrsimu/s4p100/" 
...
datagen<-function(n, p){
  ...
  y<- 1+2*X[,1]+X[,2]+0.5*X[,3]+3.8*(0.8-X[,1]*X[,1]-X[,2]*X[,2])*A+rnorm(n)
...
trueoptITR<-function(X){
  trueoptITR<-(0.8-X[,1]*X[,1]-X[,2]*X[,2]>0)*2-1   
  return(trueoptITR)
}
