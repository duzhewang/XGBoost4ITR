...
# Scenario 2: parabola decision rule  

rm(list=ls())
set.seed(2019)
funcdir="/Users/peterwang/Desktop/itrsimu/func/"
savedir<-"/Users/peterwang/Desktop/itrsimu/s2p100/" 
...
datagen<-function(n, p){
  ...
  y<- 1+2*X[,1]+X[,2]+0.5*X[,3]+1.3*(X[,2]-2*X[,1]*X[,1]+0.3)*A+rnorm(n)
...
trueoptITR<-function(X){
  trueoptITR<-(X[,2]-2*X[,1]*X[,1]+0.3>0)*2-1   
  return(trueoptITR)
}
